...
QR-ссылка должна быть вида `https://guest.<domain>/s/{storeSlug}?t=<shortToken>`:

- В production у нас разные поддомены (`guest.*`, `barista.*`, `api.*`). QR должен вести именно на `guest.*`, иначе SSR может упасть при попытке загрузить меню (ошибка Server Components render).
- Пример slug: `center` → `https://guest.<domain>/s/center`.
...
UX улучшения (DONE):
- Фильтр по категориям на экране меню (кнопки-чипы): по умолчанию показываются все категории; после выбора — показываются только выбранные, можно выбрать несколько; при выборе всех по отдельности состояние возвращается в “Все”.
- Автообновление меню гостя: обновление при возвращении фокуса/видимости вкладки + периодический refresh по таймеру, чтобы новые категории/позиции, созданные баристой, появлялись без ручного обновления страницы.
...
UX улучшения (DONE):
- Состав заказа выводится списком по строкам (каждая позиция отдельной строкой), а не одной строкой через разделители — проще читать в очереди.
...
Уточнение по реализации (DONE):
- `Notification.requestPermission()` вызываем только из user gesture (клик по кнопке на экране оплаты/статуса), иначе многие браузеры (особенно mobile) не показывают запрос.
- При переходе заказа в `ready` показываем уведомление только если разрешение уже `granted` (без фонового запроса разрешения).
- Mobile/Safari: уведомления могут быть ограничены (часто работают только в PWA, установленной на домашний экран).
